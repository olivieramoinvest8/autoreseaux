 ...
 *     "seconds_per_photo": 4, "transition": 0.6, "fps": 30, "width": 1080, "height": 1920,
 ...
 * en ouverture et en fin. Sortie : MP4 H.264, piste audio silencieuse (Instagram et Facebook l'exigent parfois),
...
async function main() {
  ...
  const W = spec.width || 1080, H = spec.height || 1920, FPS = spec.fps || 30;
  ...
  // 2) Filtre ffmpeg : un plan animé par image, puis enchaînement en fondu (xfade).
  const inputs = []; const filters = [];
  plans.forEach((pl, i) => {
    inputs.push("-loop", "1", "-framerate", String(FPS), "-t", String(pl.seconds), "-i", pl.file); // image fixe lue à FPS images/s
    const N = Math.round(pl.seconds * FPS);
    const m = pl.still ? { z: `1+0.02*on/${N}`, x: "iw/2-(iw/zoom/2)", y: "ih/2-(ih/zoom/2)" } : motion(i, N);
    // Agrandi ×2 avant le zoompan pour un mouvement sans tremblement, puis ramené à la taille de sortie.
    filters.push(`[${i}:v]scale=${W * 2}:${H * 2}:force_original_aspect_ratio=increase,crop=${W * 2}:${H * 2},zoompan=z='${m.z}':x='${m.x}':y='${m.y}':d=1:s=${W}x${H}:fps=${FPS},setsar=1,format=yuv420p[v${i}]`);
  });
  ...
  console.log(`Vidéo produite : ${path.relative(ROOT, out)} (${W}×${H}, ${total.toFixed(1)} s, ${mb} Mo, ${Math.round((Date.now() - t0) / 1000)} s de calcul)${thumb ? ` + vignette ${thumb}` : ""}`);
}
